...
const roles = [
  {
    ...
    desc: "Execute deals, manage player contracts",
    icon: Shield,
    features: [
  ...
  },
  {
    id: "SCOUT",
    label: "Scout",
    desc: "Discover talent, write reports",
    icon: Scan,
    features: [
  ...
  },
  {
    ...
    desc: "Manage player careers & brands",
    icon: Handshake,
    features: [
  ...
  },
  {
    id: "INTRODUCER",
    label: "Introducer",
    desc: "Connect players, agents & clubs",
    icon: Link2,
    features: ["Referral tracking", "Commission dashboard", "Network tools"],
  },
];
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="flex items-center justify-center gap-2 mb-10">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${s <= step ? "gradient-steel-btn text-white" : "bg-background-elevated text-foreground-dim"}`}
              >
                {s}
              </div>
              {s < 3 && (
                <div
                  className={`w-12 h-0.5 ${s < step ? "gradient-steel" : "bg-border"}`}
                />
              )}
            </div>
          ))}
        ...
        {step === 1 && (
          <div>
            <h1 className="text-2xl font-bold text-center mb-2">
              Join the future of football representation
            </h1>
            <p className="text-foreground-muted text-center mb-8 text-sm">
              No experience required — we&apos;ll give you the tools, training,
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
              {[
                ...
                <div
                  key={b.label}
                  className="card rounded-xl p-3 flex items-center gap-2.5"
                >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {role.features.map((f) => (
                          <span
                            key={f}
                            className="text-xs px-2 py-0.5 rounded bg-background-elevated text-foreground-dim"
                          >
                            {f}
                          </span>
                        ))}
        ...
        {step === 2 && (
          <div>
            ...
            <p className="text-foreground-muted text-center mb-8 text-sm">
              Signing up as{" "}
              <span className="text-accent capitalize">{form.role}</span> ·{" "}
              <button
                onClick={() => setStep(1)}
        ...
        {step === 3 && (
          <div className="text-center">
            ...
            <p className="text-foreground-muted mb-8 text-sm max-w-sm mx-auto">
              ...
              the link to confirm your account and get started.
            </p>
            <Link
              href="/login"
